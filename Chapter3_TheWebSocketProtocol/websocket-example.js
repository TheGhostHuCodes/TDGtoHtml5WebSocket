...
var WebSocketConnection = function(request, socket, upgradeHeader) {
  var self = this;

  var key = hashWebSocketKey(request.headers["sec-websocket-key"]);
...
WebSocketConnection.prototype.send = function(object) {
  var opcode;
  var payload;
  if (Buffer.isBuffer(object)) {
    ...
    payload = new Buffer(object, "utf8");
  } else {
...
WebSocketConnection.prototype._processBuffer = function() {
  ...
  var fin = b1 & 0x80;  // low four bits.
  var opcode = b1 & 0x0f;
  var b2 = buf.readUInt8(1);
  var mask = b2 & 0x08;
  var length = b2 & 0x7f; // lower 7 bits.
  ...
  if (length === 126) {
    ...
    // discard high 4 bits because this surver cannot handle huge lengths.
...
var encodeMessage = function(opcode, payload) {
  ...
  // Second bye: mask and length part 1
  ...
  if (length < 126) {
    ...
    // Note: this implementation cannot handle lengths greater than 2^32. The
    // 32-bit length is prefixed with 0x0000.
    buf.writeUInt16BE(0, 2);
    buf.writeUInt16BE(length, 6);
    payload.copy(buf, 10);
  }
  return buf;
}
...
exports.listen = function(port, host, connectionHandler) {
  var srv = http.createServer(function(request, result) {
  });

  srv.on('upgrade', function(request, socket, upgradeHeader) {
    var ws = new WebSocketConnection(request, socket, upgradeHeader);
